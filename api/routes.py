...
def buy_stock(
    ...
    try:
        with get_db() as conn:
            wallet    = Wallet(current_user["id"], conn)
            portfolio = Portfolio(current_user["id"], conn, wallet, market)
...
def get_wallet(current_user: dict = Depends(get_current_user)):
    """FIX: Was missing. Frontend calls /wallet to show cash balance."""
    with get_db() as conn:
        row = conn.execute(
            "SELECT balance FROM wallets WHERE user_id=?", (current_user["id"],)
        ).fetchone()
    ...
    with get_db() as conn:
        resets = conn.execute(
            """SELECT COUNT(*) as n FROM trades WHERE user_id=?
               AND action='RESET' AND date(timestamp)=date('now')""",
            (user_id,)
        ).fetchone()
        if resets and resets["n"] >= 3:
            raise HTTPException(429, "Maximum 3 resets per day")

        conn.execute(
            "UPDATE wallets SET balance=? WHERE user_id=?", (STARTING_BALANCE, user_id)
        )
        conn.execute("DELETE FROM holdings WHERE user_id=?", (user_id,))
        conn.execute(
            "INSERT INTO trades (user_id, action, ticker, quantity, price, timestamp) VALUES (?,?,?,?,?,datetime('now'))",
            (user_id, "RESET", "—", 0, 0)
        )
    return {
        "message": f"Portfolio reset to {APP_CURRENCY_SYMBOL}{STARTING_BALANCE:,.0f}",
...
# ══════════════════════════════════════════════════════════════════════
# WATCHLIST — POST /watchlist?ticker=AAPL (query param style)
# ══════════════════════════════════════════════════════════════════════
